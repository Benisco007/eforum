...
class _OtherProfileScreenState
  ...
  Future<void> _loadUser() async {
    try {
      ...
      if (doc.exists && mounted) {
        ...
        // Vérifier si déjà suivi via UserRepository
  ...
  Future<void> _toggleFollow() async {
    ...
    try {
      ...
      // Recharger les abonnements du feed après follow/unfollow
      await ref.read(feedViewModelProvider.notifier).refreshFollowingAndReload();
      // Mettre à jour l'état local immédiatement
      if (mounted) {
        setState(() {
          _isFollowing = !_isFollowing;
          // Mettre à jour le compteur affiché localement
          if (_user != null) {
            // Après toggle : _isFollowing est DÉJÀ inversé, donc on lit l'ancienne valeur
            final delta = _isFollowing ? 1 : -1;
            _user = _user!.copyWith(
              followersCount: (_user!.followersCount + delta).clamp(0, 999999),
            );
          }
        });
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Erreur lors du suivi', style: TextStyle(color: _txt)),
  ...
  Widget build(BuildContext context) {
    ...
    if (_user == null) {
      return Scaffold(
        ...
        body: const Center(
          child: Text('Utilisateur introuvable', style: TextStyle(color: _mut)),
        ),
      );
    }
                ...
                child: Container(
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                      ...
                      colors: [Colors.transparent, _ink.withOpacity(0.85)],
                    ),
                  ),
                ),
              ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        child: user.photoURL == null
                            ? const Icon(Icons.person_rounded, color: _neon, size: 36)
                            : null,
                      ),
                      const Spacer(),
                      // Bouton message
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: const Icon(Icons.mail_outline_rounded, color: _txt, size: 18),
                        ),
                      ),
                      ...
                      GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10),
                          decoration: BoxDecoration(
                            color: _isFollowing ? Colors.transparent : _neon,
                            borderRadius: BorderRadius.circular(12),
                            border: Border.all(color: _isFollowing ? _line : _neon),
                          ),
                  ...
                  // Nom
                  Text(user.username, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: _txt)),
                  Text('@${user.username}', style: const TextStyle(fontSize: 13, color: _mut)),
                  if (user.bio != null && user.bio!.isNotEmpty) ...[
                    const SizedBox(height: 8),
                    Text(user.bio!, style: const TextStyle(fontSize: 13, color: _txt, height: 1.5)),
                  ],
                  const SizedBox(height: 16),
                  // Stats
                  Row(
                    children: [
                      _StatItem(value: user.followersCount.toString(), label: 'Abonnés'),
                      const SizedBox(width: 24),
                      _StatItem(value: user.followingCount.toString(), label: 'Abonnements'),
                    ],
                  ),
                  if (user.favPlayerName != null) ...[
                    const SizedBox(height: 14),
                      ...
                      child: Row(
                        children: [
                          ...
                          Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('Joueur préféré', style: TextStyle(fontSize: 11, color: _mut)),
                              Text(user.favPlayerName!, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: _txt)),
                            ],
                          ),
                        ],
                      ),
          ...
          SliverPersistentHeader(
            ...
            delegate: _TabBarDelegate(
              TabBar(
                ...
                indicatorSize: TabBarIndicatorSize.label,
                dividerColor: Colors.transparent,
                tabs: const [Tab(text: 'Posts'), Tab(text: 'Builds')],
              ),
            ),
          ),
        ],
        body: TabBarView(
          ...
          children: [
            ...
            StreamBuilder<List<PostModel>>(
              stream: PostRepository().getUserPosts(widget.uid),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                if (posts.isEmpty) {
                  return Center(
                    child: Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.article_outlined, color: _neon.withOpacity(0.3), size: 48),
                        const SizedBox(height: 14),
                        const Text('Aucun post encore', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: _txt)),
                        const SizedBox(height: 6),
                        const Text('Cet utilisateur n\'a pas encore publié.', style: TextStyle(fontSize: 13, color: _mut)),
                      ],
                    ),
                  );
                }
            ...
            // ── Onglet Builds ─────────────────────────────────────────────────
            StreamBuilder<List<BuildModel>>(
              stream: BuildRepository().getUserBuilds(widget.uid),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const Center(child: CircularProgressIndicator(color: _build, strokeWidth: 2));
                }
                final builds = snapshot.data ?? [];
                if (builds.isEmpty) {
                  return Center(
                    child: Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.bolt_outlined, color: _build.withOpacity(0.3), size: 48),
                        const SizedBox(height: 14),
                        const Text('Aucun build encore', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: _txt)),
                        const SizedBox(height: 6),
                        const Text('Cet utilisateur n\'a pas encore partagé de build.', style: TextStyle(fontSize: 13, color: _mut)),
                      ],
                    ),
                  );
                }
                return ListView.builder(
                  ...
                  itemBuilder: (ctx, i) => Padding(
                    ...
                    child: BuildCard(build: builds[i]),
                  ),
                );
              },
            ),
